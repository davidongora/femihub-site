...
const ChatbotModal = () => {
  ...
  const OpenChat = () => {
    const WaURL = `https://wa.me/+256743799700?text=${encodeURIComponent(
      chatMessage
    )}`;
        ...
        <div className="fixed bottom-16 right-4 border border-custom-pink bg-white rounded-lg shadow-lg p-6 max-w-xs w-full z-50">
          ...
          <div>
            <input
              ...
              value={chatMessage}
              onChange={(e) => setChatMMessage(e.target.value)}
            />
            <button
              className="mt-2 w-full bg-custom-pink text-white p-2 rounded-md hover:bg-pink-700 focus:outline-none focus:ring-2 focus:ring-purple-600 focus:ring-opacity-50"
              onClick={OpenChat}
            >
              Send
            </button>
